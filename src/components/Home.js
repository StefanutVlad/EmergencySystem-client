  ...
  useEffect(() => {
    isLoggedIn
      ? axios.get("/user/data").then((response) => {
          //if logged in
          const usersNr = response.data.length;

          response.data.map((user, index) => {
            //console.log(response.data[index]);

            //username value
            const userUsername = Object.values(response.data[index])[3];
            //check username
            response.data.filter((username) =>
              userUsername === currentUser.username
                ? (setUserId(index),
                  setUserEntries(Object.values(user).length),
                  console.log(user.username + "s values"))
                : ""
            );
          });

          console.log("current user: " + currentUser.username);
          console.log(currentUser.username + " id in db: " + userId);
          console.log("users entries: " + userEntries);

          setUsersNumber(usersNr);
        })
      : axios.get("/user/data").then((response) => {
          //if not logged in
          const usersNr = response.data.length;
            ...
            <div className="col-lg-9 z-index-2 border-radius-xl mt-n10 mx-auto py-3 blur shadow-blur ">
              <div className="row">
                ...
                <div className="col-md-4 margin-count">
                  <div className="p-3 text-center">
                    <h1 className="text-gradient text-primary">
                      <CountUp id="state2" start={0} end={userEntries}>
                        {({ countUpRef }) => (
                          <div>
                            <span ref={countUpRef} />
                          </div>
